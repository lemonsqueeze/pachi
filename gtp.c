...
#ifdef JOSEKIFIX
/* Usage: pachi-set_external_engine_mode q1 q2 q3 q4
 * Set external joseki engine mode (one number per quadrant = number of moves left). */
static enum parse_code
cmd_pachi_set_external_engine_mode(board_t *b, engine_t *e, time_info_t *ti, gtp_t *gtp)
{
	int mode[4];
	for (int i = 0; i < 4; i++) {
...
#ifdef JOSEKIFIX
	{ "set_external_engine_mode",	cmd_pachi_set_external_engine_mode },
#endif
...
#ifdef JOSEKIFIX
	{ "pachi-set_external_engine_mode", cmd_pachi_set_external_engine_mode },
#endif
